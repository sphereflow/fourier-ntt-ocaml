...
let idft2d y =
  let n = y.rows in
  let w = dft_matrix n in
  let wbar = make n n @@ fun j k -> Complex.conj (get w j k) in
  let inv_n2 = 1.0 /. (float n *. float n) in
  let raw = mul wbar (mul y (transpose wbar)) in
  make n n @@ fun i j -> Complex.mul (get raw i j) { re = inv_n2; im = 0.0 }
...
let magnitudes m =
  ...
  let max_norm =
    Array.fold_left (fun acc c -> Float.max acc (Complex.norm c)) 0.0 m.data
  in
  let scale =
    if max_norm > 0.0 then 1.0 /. Float.log10 (1.0 +. max_norm) else 1.0
  in
